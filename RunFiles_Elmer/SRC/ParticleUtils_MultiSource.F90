...
RETURN
  ...
  SUBROUTINE ParticleAdvectParallel(Particles, SentField, RecvField, dofs ) 
    ...
    PRINT*, 'We are in ParticleAdvectParallel for partition', ParEnv % MyPe + 1 !!Debug
...
100 SELECT CASE ( family )
  ...
  SUBROUTINE InitializeParticles( Particles, InitParticles, AppendParticles, &
    ...
    IF( ListCheckPresentAnyBodyForce( CurrentModel,&
        'Particle Distance Integral Source 1') ) THEN
      CALL ParticleVariableCreate( Particles,'Particle Distance Integral 1' )
      CALL ParticleVariableInitialize( Particles, Mesh,'Particle Distance Integral 1')
    END IF
    IF( ListCheckPresentAnyBodyForce( CurrentModel,&
        'Particle Distance Integral Source 2') ) THEN
      CALL ParticleVariableCreate( Particles,'Particle Distance Integral 2' )
      CALL ParticleVariableInitialize( Particles, Mesh,'Particle Distance Integral 2')
    END IF
    IF( ListCheckPresentAnyBodyForce( CurrentModel,&
        'Particle Time Integral Source 1') ) THEN
      CALL ParticleVariableCreate( Particles,'Particle Time Integral 1' )
      CALL ParticleVariableInitialize( Particles, Mesh,'Particle Time Integral 1')
    END IF
    IF( ListCheckPresentAnyBodyForce( CurrentModel,&
        'Particle Time Integral Source 2') ) THEN
      CALL ParticleVariableCreate( Particles,'Particle Time Integral 2' )
      CALL ParticleVariableInitialize( Particles, Mesh,'Particle Time Integral 2')
    END IF
...
!------------------------------------------------------------ 
 ...
 END SUBROUTINE ParticleAdvanceTimestep
  ...
  SUBROUTINE ParticlePathIntegral( Particles, RKstepInput )
    ...
    TYPE(Variable_t), POINTER :: TimeIntegVar1, DistIntegVar1, TimeIntegVar2, &
        DistIntegVar2, DtVar, PartTimeVar, MeshDtVar 
    LOGICAL :: GotVar, RK2
    ...
    TYPE(Element_t), POINTER :: Element
    TYPE(Mesh_t), POINTER :: Mesh
    LOGICAL :: TimeInteg1 = .FALSE., DistInteg1 = .FALSE., TimeInteg2 = .FALSE., &
        DistInteg2 = .FALSE., UseGradSource1 = .FALSE., UseGradSource2 = .FALSE., &
        TimeDepFields1 = .FALSE., TimeDepFields2 = .FALSE.
    TYPE(ValueHandle_t), SAVE :: TimeSource_h1, DistSource_h1, TimeSource_h2, DistSource_h2
    CHARACTER(:), ALLOCATABLE :: VariableName
    CHARACTER(*), PARAMETER :: Caller = 'ParticlePathIntegral'


    SAVE TimeInteg1, DistInteg1, TimeInteg2, DistInteg2, dim, Visited, Mesh, &
        DtVar, Basis, Nodes, Params, TimeIntegVar1, DistIntegVar1, &
        TimeIntegVar2, DistIntegVar2, UseGradSource1, UseGradSource2, &
        dBasisdx, TimeDepFields1, TimeDepFields2, PartTimeVar, &
        MeshDtVar, UseDummy
    ...
    IF(.NOT. Visited ) THEN
      ...
      TimeIntegVar1 => ParticleVariableGet( Particles,'particle time integral 1')
      TimeInteg1 = ASSOCIATED( TimeIntegVar1 )
      IF( TimeInteg1 ) THEN    
        IF( .NOT. ListCheckPresentAnyBodyForce( CurrentModel,&
            'Particle Time Integral Source 1') ) THEN
          CALL Fatal(Caller,'Path integral requires body force: "Particle Time Integral Source 1"')
        END IF
      END IF
      
      DistIntegVar1 => ParticleVariableGet( Particles,'particle distance integral 1')
      DistInteg1 = ASSOCIATED( DistIntegVar1 )
      IF( DistInteg1 ) THEN
        IF( .NOT. ListCheckPresentAnyBodyForce( CurrentModel,&
            'Particle Distance Integral Source 1') ) THEN
          CALL Fatal(Caller,'Path integral requires body force: "Particle Distance Integral Source 1"')
        END IF
      END IF

      TimeIntegVar2 => ParticleVariableGet( Particles,'particle time integral 2')
      TimeInteg2 = ASSOCIATED( TimeIntegVar2 )
      IF( TimeInteg2 ) THEN        
        IF( .NOT. ListCheckPresentAnyBodyForce( CurrentModel,&
            'Particle Time Integral Source 2') ) THEN
          CALL Fatal(Caller,'Path integral requires body force: "Particle Time Integral Source 2"')
        END IF
      END IF
      
      DistIntegVar2 => ParticleVariableGet( Particles,'particle distance integral 2')
      DistInteg2 = ASSOCIATED( DistIntegVar2 )
      IF( DistInteg2 ) THEN
        IF( .NOT. ListCheckPresentAnyBodyForce( CurrentModel,&
            'Particle Distance Integral Source 2') ) THEN
          CALL Fatal(Caller,'Path integral requires body force: "Particle Distance Integral Source 2"')
        END IF
      END IF

      IF( .NOT. (TimeInteg1 .OR. DistInteg1 .OR. &
                 TimeInteg2 .OR. DistInteg2 ) ) RETURN
      ...
      UseGradSource1 = ListGetLogical( Params,'Source Gradient Correction 1',Found)
      UseGradSource2 = ListGetLogical( Params,'Source Gradient Correction 2',Found)
      ! If the correction is not given follow the logic of velocity estimation
      IF( (UseGradSource1 .OR. UseGradSource2) .AND. Particles % RK2 ) THEN
        CALL Fatal(Caller,'Quadratic source correction incompatibe with Runge-Kutta')
      END IF

      TimeDepFields1 = ListGetLogical( Params,'Source Time Correction 1',Found ) 
      TimeDepFields2 = ListGetLogical( Params,'Source Time Correction 2',Found ) 
      IF(TimeDepFields1 .OR. TimeDepFields2 ) THEN
      ...
      IF( UseDummy .AND. (UseGradSource1 .OR. USeGradSource2) ) THEN
    ...
    IF(TimeInteg1) THEN
      CALL ListInitElementKeyword( TimeSource_h1,'Body Force','Particle Time Integral Source 1',DummyCount=j)    
    END IF
    
    IF(DistInteg1) THEN
      CALL ListInitElementKeyword( DistSource_h1,'Body Force','Particle Distance Integral Source 1',DummyCount=j)
    END IF

    IF(TimeInteg2) THEN
      CALL ListInitElementKeyword( TimeSource_h2,'Body Force','Particle Time Integral Source 2',DummyCount=j)    
    END IF 

    IF(DistInteg2) THEN
      CALL ListInitElementKeyword( DistSource_h2,'Body Force','Particle Distance Integral Source 2',DummyCount=j)
    END IF
    ...
    DO No=1, NoParticles
      ...
      IF( UseGradSource1 .OR. UseGradSource2 ) THEN
      ...
      ! Path integral over time on variable 1
      IF( TimeInteg1 ) THEN       
        IF( UseDummy ) THEN
          DummyVals(1) = TimeIntegVar1 % Values(No)
          Source = ListGetElementReal( TimeSource_h1, Basis, Element, Found, DummyVals = DummyVals )         
        ELSE
          Source = ListGetElementReal( TimeSource_h1, Basis, Element, Found )
        END IF
        IF( Found ) THEN
          IF ( UseGradSource1 ) THEN
            GradSource = ListGetElementRealGrad( TimeSource_h1,dBasisdx,Element)                    
            Source = Source + 0.5*SUM( GradSource(1:dim) * (PrevCoord(1:dim) - Coord(1:dim)) )
          END IF

          IF( TimeDepFields1 ) THEN
            IF( UseDummy ) THEN
              PrevSource = ListGetElementReal( TimeSource_h1, Basis, Element, Found,tstep=-1, DummyVals = DummyVals  )
            ELSE
              PrevSource = ListGetElementReal( TimeSource_h1, Basis, Element, Found,tstep=-1 )
            END IF
            IF ( UseGradSource1 ) THEN
              GradSource = ListGetElementRealGrad( TimeSource_h1,dBasisdx,Element,tstep=-1)                    
              PrevSource = PrevSource + 0.5*SUM( GradSource(1:dim) * (PrevCoord(1:dim) - Coord(1:dim)) )
            END IF
            IF ( Particles % DtSign == 1) THEN
              TimeIntegVar1 % Values(No) = TimeIntegVar1 % Values(No) + dtime * &
                ( (1-dtrat)*PrevSource + dtrat * Source ) 
            ELSE
              TimeIntegVar1 % Values(No) = TimeIntegVar1 % Values(No) + dtime * &
                ( (1-dtrat)*Source + dtrat * PrevSource ) 
            END IF
          ELSE          
            TimeIntegVar1 % Values(No) = TimeIntegVar1 % Values(No) + dtime * Source
            IF( No == 1500 ) THEN
            END IF
          END IF
        END IF        
      ...
      ! Path integral over time on variable 2
      IF( TimeInteg2 ) THEN        
        IF( UseDummy ) THEN
          DummyVals(1) = TimeIntegVar2 % Values(No)
          Source = ListGetElementReal( TimeSource_h2, Basis, Element, Found, DummyVals = DummyVals )         
        ELSE
          Source = ListGetElementReal( TimeSource_h2, Basis, Element, Found )
        END IF
        IF( Found ) THEN
          IF ( UseGradSource2 ) THEN
            GradSource = ListGetElementRealGrad( TimeSource_h2,dBasisdx,Element)                    
            Source = Source + 0.5*SUM( GradSource(1:dim) * (PrevCoord(1:dim) - Coord(1:dim)) )
          END IF

          IF( TimeDepFields2 ) THEN
            IF( UseDummy ) THEN
              PrevSource = ListGetElementReal( TimeSource_h2, Basis, Element, Found,tstep=-1, DummyVals = DummyVals  )
            ELSE
              PrevSource = ListGetElementReal( TimeSource_h2, Basis, Element, Found,tstep=-1 )
            END IF
            IF ( UseGradSource2) THEN
              GradSource = ListGetElementRealGrad( TimeSource_h2,dBasisdx,Element,tstep=-1)                    
              PrevSource = PrevSource + 0.5*SUM( GradSource(1:dim) * (PrevCoord(1:dim) - Coord(1:dim)) )
            END IF
            IF ( Particles % DtSign == 1) THEN
              TimeIntegVar2 % Values(No) = TimeIntegVar2 % Values(No) + dtime * &
                ( (1-dtrat)*PrevSource + dtrat * Source ) 
            ELSE
              TimeIntegVar2 % Values(No) = TimeIntegVar2 % Values(No) + dtime * &
                ( (1-dtrat)*Source + dtrat * PrevSource ) 
            END IF
          ELSE          
            TimeIntegVar2 % Values(No) = TimeIntegVar2 % Values(No) + dtime * Source
          END IF
        END IF        
      END IF
      
      ! Path integral over distance for variable 1
      IF( DistInteg1) THEN
        IF( RK2 ) THEN
        ...
        IF( UseDummy ) THEN
          DummyVals(1) = DistIntegVar1 % Values(No)
          Source = ListGetElementReal( DistSource_h1, Basis, Element, Found, DummyVals = DummyVals )         
        ELSE
          Source = ListGetElementReal( DistSource_h1, Basis, Element, Found )
        END IF

        IF( Found ) THEN
          IF ( UseGradSource1 ) THEN
            GradSource = ListGetElementRealGrad( DistSource_h1,dBasisdx,Element)                  
            Source = Source + 0.5*SUM( GradSource(1:dim) * (PrevCoord(1:dim) - Coord(1:dim)) )
          END IF          
          IF( TimeDepFields1 ) THEN
            IF( UseDummy ) THEN
              PrevSource = ListGetElementReal( DistSource_h1, Basis, Element, Found,tstep=-1, DummyVals = DummyVals  )
            ELSE
              PrevSource = ListGetElementReal( DistSource_h1, Basis, Element, Found,tstep=-1  )
            END IF
            IF ( UseGradSource1 ) THEN
              GradSource = ListGetElementRealGrad( DistSource_h1,dBasisdx,Element,tstep=-1)                    
              PrevSource = PrevSource + 0.5*SUM( GradSource(1:dim) * (PrevCoord(1:dim) - Coord(1:dim)) )
            END IF
            IF ( Particles % DtSign == 1) THEN
               DistIntegVar1 % Values(No) = DistIntegVar1 % Values(No) + ds * &
                   ( (1-dtrat)*PrevSource + dtrat * Source ) 
            ELSE
               DistIntegVar1 % Values(No) = DistIntegVar1 % Values(No) + ds * &
                   ( (1-dtrat)*Source + dtrat * PrevSource ) 
            END IF
          ELSE          
            DistIntegVar1 % Values(No) = DistIntegVar1 % Values(No) + ds * Source
          END IF
        END IF
      END IF
      
      ! Path integral over distance for variable 2
      IF( DistInteg2) THEN
        IF( RK2 ) THEN
          ! for R-K the velocity has been updated to the midpoint and this is used
          ! to determine the differential path. 
        ...
        IF( UseDummy ) THEN
          DummyVals(1) = DistIntegVar2 % Values(No)
          Source = ListGetElementReal( DistSource_h2, Basis, Element, Found, DummyVals = DummyVals )         
        ELSE
          Source = ListGetElementReal( DistSource_h2, Basis, Element, Found )
        END IF

        IF( Found ) THEN
          IF ( UseGradSource2 ) THEN
            GradSource = ListGetElementRealGrad( DistSource_h2,dBasisdx,Element)                  
            Source = Source + 0.5*SUM( GradSource(1:dim) * (PrevCoord(1:dim) - Coord(1:dim)) )
          END IF          
          IF( TimeDepFields2 ) THEN
            IF( UseDummy ) THEN
              PrevSource = ListGetElementReal( DistSource_h2, Basis, Element, Found,tstep=-1, DummyVals = DummyVals  )
            ELSE
              PrevSource = ListGetElementReal( DistSource_h2, Basis, Element, Found,tstep=-1  )
            END IF
            IF ( UseGradSource2 ) THEN
              GradSource = ListGetElementRealGrad( DistSource_h2,dBasisdx,Element,tstep=-1)                    
              PrevSource = PrevSource + 0.5*SUM( GradSource(1:dim) * (PrevCoord(1:dim) - Coord(1:dim)) )
            END IF
            IF ( Particles % DtSign == 1) THEN
               DistIntegVar2 % Values(No) = DistIntegVar2 % Values(No) + ds * &
                   ( (1-dtrat)*PrevSource + dtrat * Source ) 
            ELSE
               DistIntegVar2 % Values(No) = DistIntegVar2 % Values(No) + ds * &
                   ( (1-dtrat)*Source + dtrat * PrevSource ) 
            END IF
          ELSE          
            DistIntegVar2 % Values(No) = DistIntegVar2 % Values(No) + ds * Source
          END IF
        END IF
      END IF
