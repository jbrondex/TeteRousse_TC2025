...
1 CONTINUE
  ...
  DO i=1,nstep
    IF (Particles % DtSign == -1) THEN
      PRINT*, 'Backward Cycle, Internal step n°:', i
    ELSE
      PRINT*, 'Forward Cycle, Internal step n°:', i
    END IF
...
100 NoVar = 0 
    DO WHILE(.TRUE.)
      ...
      IF( ReverseTime ) THEN
        ...
        ELSE IF( VariableName == 'particle time integral 1' .OR. &
            VariableName == 'particle time integral 2' .OR. &
            VariableName == 'particle distance integral 1' .OR. &
            VariableName == 'particle distance integral 2' ) THEN
      ...
      IF( InternalVariable .AND. .NOT. InitIntegral ) THEN
        ...
        ELSE IF( SEQL(VariableName, 'particle') ) THEN
          ...
          IF( ASSOCIATED( ParticleVar ) ) THEN
            PRINT*, 'Size of NewValues =',SIZE(NewValues), 'in partition', ParEnv % MyPe + 1 !!Debug
            PRINT*, 'Size of ParticleVar =', SIZE(ParticleVar % Values(:)), 'in partition', ParEnv % MyPe + 1 !!Debug
!            NewValues = ParticleVar % Values(1:SIZE(NewValues)) 
             NewValues(1:NoParticles) = ParticleVar % Values(1:NoParticles) !!Debug TEST
            PRINT*, 'Right after aftecting newvalues to particle values in partition', ParEnv % MyPe + 1 !!Debug
          ELSE
      ...
      !---------------------------------------------------------------------
      IF( Parallel ) THEN
        PRINT*, 'We go in parallel loop for partition', ParEnv % MyPe + 1 !!Debug
        NodeValues = 0._dp
